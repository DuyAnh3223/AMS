...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col md:flex-row">
      ...
      {/* SIDEBAR */}
      <aside
        className={`fixed inset-y-0 left-0 border-r border-slate-200 bg-white z-40 transition-all duration-300 md:translate-x-0 md:static md:flex md:flex-col relative ${
          isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
        } ${isCollapsed ? 'md:w-20' : 'md:w-72'}`}
      >
        {/* Sidebar Header */}
        <div className={`flex h-20 items-center border-b border-slate-200 relative ${
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50/40">
          <div className={`flex items-center gap-3 rounded-2xl bg-white border border-slate-200 shadow-sm transition-all ${
            ...
            {!isCollapsed && (
              <div className="flex-1 min-w-0 animate-in fade-in duration-200">
                ...
                <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 mt-1 text-[10px] font-bold uppercase tracking-wider ${
                  userRole === 'admin' 
