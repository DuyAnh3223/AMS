...
export default function DashboardPage() {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const partners = await apiRequest('/partners/all');
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold bg-indigo-50 text-indigo-600 border border-indigo-100 shadow-sm">
              ...
              <span>Quyền hạn: {user?.role.toUpperCase()}</span>
            </div>
          ...
          <div className="flex items-center gap-4 shrink-0">
            <button
              onClick={fetchStats}
              disabled={loading}
              className="h-10 w-10 rounded-xl bg-white hover:bg-slate-50 flex items-center justify-center text-slate-400 hover:text-slate-700 border border-slate-200 shadow-sm transition-colors disabled:opacity-50"
            >
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 leading-normal">Mọi hành động thay đổi dữ liệu hoặc truy cập trang đều được chặn lọc tự động ở cấp độ cổng ngầm định tuyến Next.js.</p>
                </div>
              </div>
            </div>
          </div>

          <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-455">
